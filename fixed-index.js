// index.js - Porsuk Ticket Bot - Basit Hafıza/Memory Sürümü - Hata Çözümlü
...
async function handleTicketCreation(message, categoryId, description) {
  try {
    ...
    try {
      ...
      const staffRoleId = message.guild.roles.cache.find(role => 
        role.name.toLowerCase().includes('staff') || 
        role.name.toLowerCase().includes('yetkili') || 
        role.name.toLowerCase().includes('mod'))?.id || message.guild.id;
    ...
    if (interaction.isButton()) {
      ...
      if (interaction.customId === 'ticket_category') {
        ...
        const message = await interaction.channel.send({
          content: `<@${interaction.user.id}>, lütfen ticket açıklamanızı buraya yazın:`
        });
        ...
        try {
          ...
          console.error('Error awaiting messages:', error);
          interaction.followUp({ 
            content: 'Ticket açıklaması için süre doldu. Lütfen tekrar deneyin.',
            ephemeral: true
          });
        }
      }
